...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    const newIndex = Math.max(0, Math.floor(latest * data.length) - 1);
    if (newIndex !== activeIndex && newIndex < data.length) {
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-10">
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-20 md:top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3
                className={`hidden md:block text-xl md:pl-20 md:text-5xl font-bold transition-colors duration-300 ${index === activeIndex
                  ? "text-purple-500"
                  : "text-neutral-500"
                  }`}
              >
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3
                className={`md:hidden block text-2xl mb-4 text-left font-bold transition-colors duration-300 ${index === activeIndex
                  ? "text-purple-500"
                  : "text-neutral-500"
                  }`}
              >
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0 w-[2px] bg-gradient-to-t from-purple-500 via-violet-500 to-pink-500 from-[0%] via-[10%] rounded-full"
          />
        </div>
      </div>
    </div>
